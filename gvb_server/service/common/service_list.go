...
func ImageList[T any](model T, option Option) (list []T, count int64, err error) {
	DB := global.DB
	if option.Debug {
		DB = global.DB.Debug().Session(&gorm.Session{Logger: global.MysqlLog})
	}
	...
	if option.Key != "" {
		count = DB.Debug().Select("id").Where("name like ?", option.Key+"%").Find(&list).RowsAffected
	...
	if option.Role != "" {
		count = DB.Debug().Select("id").Where("image_type = ? ", option.Role).Find(&list).RowsAffected

	}

	if option.Key != "" && option.Role != "" {
		count = DB.Debug().Select("id").Where("name like ? and role = ?", option.Key+"%", option.Role).Find(&list).RowsAffected
	...
	if option.Key != "" {
		err = DB.Debug().Limit(option.Limit).Offset(offset).Order(option.Sort).Where("name like ?", option.Key+"%").Find(&list).Error
	...
	if option.Role != "" {
		err = DB.Debug().Limit(option.Limit).Offset(offset).Order(option.Sort).Where("image_type = ? ", option.Role).Find(&list).Error

	}

	if option.Key != "" && option.Role != "" {
		err = DB.Debug().Limit(option.Limit).Offset(offset).Order(option.Sort).Where("name like ? and image_type = ?", option.Key+"%", option.Role).Find(&list).Error
